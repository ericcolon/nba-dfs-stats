UPDATE Schedule SET OverUnder = 200 WHERE OverUnder is Null;
UPDATE Schedule SET Spread = 0 WHERE Spread is Null;
UPDATE Primetime SET OverUnder = 200 WHERE OverUnder is Null;
UPDATE Primetime SET Spread = 0 WHERE Spread is Null;

DROP TABLE IF EXISTS TodayTrainingData;
CREATE TABLE TodayTrainingData
SELECT 
...
FROM (SELECT s.Date AS CurrDate, 
        s.GameID, 
        p.PlayerID, 
        pg.GameID AS Prev_Game_ID,
        p.TeamID
      FROM Schedule s
      INNER JOIN Players p
        ON (p.TeamID = s.HomeTeamID OR p.TeamID = s.AwayTeamID)
      INNER JOIN PlayerGames pg
        ON p.PlayerID = pg.PlayerID
        AND pg.Next_Game_ID IS NULL
      WHERE s.Date=DATE(NOW())) a
...
(SELECT GameID, Date, HomeTeamID, AwayTeamID, OverUnder, Spread 
 ...
 GROUP BY 1, 2, 3, 4, 5) h
  ON a.CurrDate = h.Date
  AND (a.TeamID = h.HomeTeamID OR a.TeamID = h.AwayTeamID)
INNER JOIN Salaries i
  ON a.CurrDate = i.Date
  AND a.PlayerID = i.PlayerID
  AND i.System = 'Draftkings'
INNER JOIN Salaries j
  ON b.Date = j.Date
  AND b.PlayerID = j.PlayerID
  AND j.System = 'Draftkings') prev
INNER JOIN
(SELECT s.Date AS CurrDate, p.PlayerID
FROM Schedule s
INNER JOIN Players p
  ON (p.TeamID = s.HomeTeamID OR p.TeamID = s.AwayTeamID)
WHERE s.Date=DATE(NOW())) curr
ON prev.CurrDate = curr.CurrDate
AND prev.PlayerID = curr.PlayerID
;

ALTER TABLE TodayTrainingData ADD INDEX (HomeTeamID);
ALTER TABLE TodayTrainingData ADD INDEX (AwayTeamID);
ALTER TABLE TodayTrainingData ADD INDEX (CurrDate);
